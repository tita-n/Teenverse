...
const initialItems = [
  ...
  { name: 'Bear', category: 'animal', price: 300, image_url: 'https://i.postimg.cc/653w4kH5/image-fx-8.png perplexity', description: 'A cuddly virtual bear companion.', is_limited: 0 },
...
initialItems.forEach((item) => {
  db.execute({
    sql: `INSERT OR IGNORE INTO shop_items (name, category, price, image_url, description, is_limited, stock) VALUES (?, ?, ?, ?, ?, ?, ?)`,
    args: [item.name, item.category, item.price, item.image_url, item.description, item.is_limited, item.stock],
  });
});
...
db.execute("CREATE INDEX IF NOT EXISTS idx_shop_items_category ON shop_items(category)");
db.execute("CREATE INDEX IF NOT EXISTS idx_showdown_participants_tournament ON showdown_participants(tournament_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_showdown_boosts_tournament ON showdown_boosts(tournament_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_hall_of_fame_user ON hall_of_fame(user_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_hype_battles_tournament ON hype_battles(tournament_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_showdown_clips_tournament ON showdown_clips(tournament_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_showdown_clip_votes_user ON showdown_clip_votes(user_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_rants_category ON rants(category)");
db.execute("CREATE INDEX IF NOT EXISTS idx_rant_comments_rant ON rant_comments(rant_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_post_hall_of_fame_user ON post_hall_of_fame(user_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_post_comments_post ON post_comments(post_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_comment_replies_comment ON comment_replies(comment_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_comment_likes_comment ON comment_likes(comment_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_post_shares_post ON post_shares(post_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_conversations_user1 ON conversations(user1_id)");
db.execute("CREATE INDEX IF NOT EXISTS idx_conversations_user2 ON conversations(user2_id)");
...
nextMonth.setDate(1);
db.execute({
  sql: "INSERT OR IGNORE INTO showdown_tournaments (season, status, start_date) VALUES (?, ?, ?)",
  args: [`Season ${nextMonth.getFullYear()}-${nextMonth.getMonth() + 1}`, "open", nextMonth.toISOString().split("T")[0]],
});
